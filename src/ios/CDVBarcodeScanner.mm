...
//    if (!output) return @"unable to obtain video capture output";
    ...
    output.metadataObjectTypes = @[AVMetadataObjectTypeCode39Code, AVMetadataObjectTypeCode128Code, AVMetadataObjectTypeCode39Mod43Code, AVMetadataObjectTypeCode93Code];
    ...
    landscapeSize.width = self.previewLayer.bounds.size.height;
    landscapeSize.height = self.previewLayer.bounds.size.width;

    CGRect landscapeRect;
    landscapeRect.size = landscapeSize;
    landscapeRect.origin = self.previewLayer.bounds.origin;

    self.previewLayer.frame = landscapeRect;

    //CGRect bounds = self.parentViewController.view.bounds;
    // self.previewLayer.frame = CGRectMake(0, 0, bounds.size.width, bounds.size.height);
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputMetadataObjects:(NSArray *)metadataObjects fromConnection:(AVCaptureConnection *)connection
{
    ...
    for (AVMetadataObject *metadata in metadataObjects) {
        ...
        {
            [self barcodeScanSucceeded:detectionString format:metadata.type];
            break;
        }
    }
}
...
@interface DimmingOverley:UIView
@property BOOL shouldDrawRedLine;
@end

@implementation DimmingOverley:UIView
...
- (void)drawRect:(CGRect)rect {
    ...
    [[[UIColor blackColor] colorWithAlphaComponent:RETICLE_ALPHA] setFill];
    UIRectFill( rect );

    CGFloat raticleSide = rect.size.width*0.7;
    CGFloat x = rect.size.width/2.0 - raticleSide/2.0;
    CGFloat y = rect.size.height/2.0 - raticleSide/2.0;
    CGRect raticleRect = CGRectMake(x, y, raticleSide, raticleSide);

    [[UIColor clearColor] setFill];
    UIRectFill( raticleRect );

    if (self.shouldDrawRedLine) {
        UIColor* color = [UIColor colorWithRed:1.0 green:0.0 blue:0.0 alpha:RETICLE_ALPHA];
        CGContextSetStrokeColorWithColor(context, color.CGColor);
        CGContextSetLineWidth(context, RETICLE_WIDTH);
        CGContextBeginPath(context);

        CGContextMoveToPoint(context, x, rect.size.height/2.0);
        CGContextAddLineToPoint(context, x + raticleSide, rect.size.height/2.0);
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    self.processor.previewLayer.frame = rect;
}
...
- (UIView*)buildOverlayView {
    ...
    //CGRect bounds = self.currentScreenBoundsDependOnOrientation;
    ...
    CGRect dimmingRect = CGRectMake(0, 0, rootViewWidth, rootViewHeight - toolbarHeight);
    DimmingOverley* dimming = [[DimmingOverley alloc] initWithFrame:dimmingRect];
    dimming.shouldDrawRedLine = self.processor.is1D;

    dimming.opaque           = NO;
    dimming.contentMode      = UIViewContentModeScaleAspectFit;
    dimming.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight;
//   dimming.autoresizingMask = 0
//    | UIViewAutoresizingFlexibleLeftMargin
//    | UIViewAutoresizingFlexibleRightMargin
//    | UIViewAutoresizingFlexibleTopMargin
//    | UIViewAutoresizingFlexibleBottomMargin;
    [overlayView addSubview: dimming];


    //Draw flashlight buton
    UIButton *flashLightButton = [UIButton buttonWithType:UIButtonTypeCustom];
    [flashLightButton addTarget:(id)self
                         action:@selector(toggleTorch:)
               forControlEvents:UIControlEventTouchUpInside];
    [flashLightButton setImage:[UIImage imageNamed:@"FLASHLIGHT"] forState:UIControlStateNormal];
    flashLightButton.frame = CGRectMake(0, 0, 60.0, 50.0);
